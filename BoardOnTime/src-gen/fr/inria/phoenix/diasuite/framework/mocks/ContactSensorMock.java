...
public final class ContactSensorMock {
    ...
    class Proxy extends AbstractContactSensor {
        ...
        protected fr.inria.phoenix.diasuite.framework.datatype.state.State getState() throws Exception {
            fr.inria.phoenix.diasuite.framework.datatype.state.State result = __state;
            if(result == null)
                return super.getState();
            return result;
        }
        
        public void _publishState(fr.inria.phoenix.diasuite.framework.datatype.state.State newStateValue) {
            publishState(newStateValue);
        }
    }
    ...
    // Code for source state from device ContactSensor
    /**
     * Publish the value of source <code>state</code> from device <code>ContactSensor</code>.
    
    <pre>
    source state as State;
    </pre>
    @param newStateValue the new value for the source <code>state</code>
    @return this for fluent interface
     */
    public ContactSensorMock state(fr.inria.phoenix.diasuite.framework.datatype.state.State newStateValue) {
        proxy._publishState(newStateValue);
        return this;
    }
    
    private fr.inria.phoenix.diasuite.framework.datatype.state.State __state = null;
    /**
     * Set the value (without publication) of source <code>state</code> from device <code>ContactSensor</code>.
    ...
    @return this for fluent interface
     */
    public ContactSensorMock setState(fr.inria.phoenix.diasuite.framework.datatype.state.State newStateValue) {
        __state = newStateValue;
        return this;
    }
    // End of code for source state from device ContactSensor
}
